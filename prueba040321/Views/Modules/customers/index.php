...
<main role="main" class="container flex-fill "  style="max-width:69.6% !important;"> <!-- se amplía el container similar a mdb5 -->
    <div class="d-flex align-items-center p-3 my-3 text-white-50 bg-crimson rounded box-shadow ">
        ...
        <div class="row ml-auto " >
		    ...
		    <button type="button" class="btn btn-info btn-rounded btn-md waves-effect float-right" data-toggle="modal" data-target="#modalAgregar">Agregar Cliente</button>
        </div>
    </div>
    ...
    <div class="my-3 p-3 bg-white rounded box-shadow  ">
        ...
        <div class="media text-muted pt-3 table-responsive">
            ...
            <table width="100%" class="table table-bordered ">
			  ...
			  <tbody>
<?php foreach($obj_Customer as $customer_data){ ?>
			    <tr id="row<?= $customer_data->cli_id; ?>">			      
			      <td><?= $customer_data->cli_nombre; ?></td>
			      <td><?= $customer_data->cli_apellido; ?></td>
			      <td><?= $customer_data->cli_correo; ?></td>
			      <td><?= $customer_data->cli_telefono; ?></td>
			      <td><?= $customer_data->cli_fecha_creado; ?></td>
			      <td><?= $customer_data->cli_fecha_modificado; ?></td>
			      <td width="3%" data-role="status"><?= $customer_data->cli_status; ?></td>
			      <td >
			      	<div class="row col-12" > <!-- ver otra forma de arreglar el espaciado de los botones con este col-12 y abajo mr-5 wtf pendiente.. -->
				      		...
				      		<div  class="col-lg-6 "> 
					      		<a onclick="actualizar( '<?= $customer_data->cli_id; ?>' , '<?= $customer_data->cli_nombre; ?>','<?= $customer_data->cli_apellido; ?>', '<?= $customer_data->cli_correo; ?>', '<?= $customer_data->cli_telefono; ?>', '<?= $customer_data->cli_status; ?>' )"  
					      		  data-toggle="modal" 
								  type="button"
							...
							<div class="col-lg-6 " >
									...
									<a onclick="eliminarv2('<?= $customer_data->cli_id; ?>')"  
								  type="button"
    ...
    <div class="modal-content">
    	<form class="needs-validation" novalidate action="suppliers" method="POST" name="formAdd">
	      <div class="modal-header text-center">
	        <h5 class="modal-title w-100 font-weight-bold dark-grey-text my-3" id="exampleModalLongTitle">Agregar Cliente</h5>
		      	...
		      	<div class="col-md-6">
		      		...
		      		<div class="md-form mb-5">
		      			<label data-error="wrong" data-success="right" for="correo">Correo</label>
                                        <input type="email" id="correo" name="telefono" class="form-control " required>			        	
			        	<div class="invalid-feedback"> 
			        ...
			        <div class="md-form mb-5">
			        	<label data-error="wrong" data-success="right" for="direccion">Telefono</label>
			        	<input type="text" id="telefono" name="direccion" class="form-control " required>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-content">
    	<form class="needs-validation" novalidate action="suppliers" method="POST" name="formUpd">
	      <div class="modal-header text-center">
	        <h5 class="modal-title w-100 font-weight-bold dark-grey-text my-3" id="exampleModalLongTitle">Actualizar Cliente</h5>
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	          <span aria-hidden="true">&times;</span>
	        </button>
	      </div>
	      <div class="modal-body">
	      	...
	      	<div class="row form-row">
	      		<div class="col-md-6">
			        ...
			        <div class="md-form mb-5">
			        	<label data-error="wrong" data-success="right" for="updtCorreo">Apellido</label>
			        	<input type="email" id="updtCorreo" name="apellido" class="form-control " required>
		      	...
		      	<div class="col-md-6">
		      		...
		      		<div class="md-form mb-5">
		      			<label data-error="wrong" data-success="right" for="updtTelefono">Correo</label>
			        	<input type="text" id="updtTelefono" name="correo" class="form-control " required>			        	
			        	<div class="invalid-feedback"> 
				        	Por favor, ingresa un número de teléfono.
				      	</div>
			        </div>
			        ...
			        <div class="md-form mb-5">
			        	<label data-error="wrong" data-success="right" for="updtDireccion">Telefono</label>
			        	<input type="text" id="updtDireccion" name="telefono" class="form-control " required>
			  	...
			  	$.ajax({
					...
					success: function(respuesta){//va al archivo con las post y en respuesta nos manda el retorno que tenemos en el 'receptor'.
						...
						if(respuesta==1){
							...
							Swal.fire(
						      'Borrado!',
						      'El cliente id:'+id+' ha sido borrado.',
						      'success'
						    )
...
document.querySelector("#title").innerHTML += '<a href=""> Clientes</a>'
...
<script>
	function actualizar(id, nombre, apellido, correo, telefono, direccion, status){
		//alert("El id es: "+id)
		...
		$('#updtNombre').add('#updtApellido"').add('#updtCorreo').add('#updtTelefono').add('#updtStatus').trigger("change"); //label bug fixed, que no va a top
	...
	$(document).ready(function(){
		...
		$(".btnEliminar").click(function(){
			console.log("Es: "+"<?= $customer_data->cli_id; ?>")
